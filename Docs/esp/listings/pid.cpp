...
void motorTask(void*)
{
    ...
    // init engine object
    Motor engine(MOTOR_UNIT, MOTOR_IN1, MOTOR_IN2, MOTOR_TIMER); 
    
    // init encoder object
    Encoder encoder(ENC_A, ENC_B, ENC_PCNT_UNIT);  
    
    // init PID object
    Pid pid(encoder, &config);                                   
    ...
    while (true)
    {
        ...
        // set PID loop to 10ms
        vTaskDelayUntil(&xLastWakeTime, PID_LOOP_PERIOD);
    }
